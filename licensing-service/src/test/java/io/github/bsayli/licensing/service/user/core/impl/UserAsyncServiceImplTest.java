...
class UserAsyncServiceImplTest {
    ...
    void repository_generalException() {
        ...
        RuntimeException repositoryFailure =
                new RuntimeException("Unexpected repository failure while fetching user");

        when(userRepository.getUser("u")).thenThrow(repositoryFailure);

        CompletableFuture<LicenseInfo> f = svc.getUser("u");

        ExecutionException ex = assertThrows(ExecutionException.class, f::get);
        assertSame(repositoryFailure, ex.getCause());
    }
