...
public class LicenseEvaluationServiceImpl implements LicenseEvaluationService {
    ...
    public LicenseEvaluationServiceImpl(UserOrchestrationService userService,
                                        LicensePolicyValidator licenseValidationService) {
    ...
    public LicenseInfo evaluateLicense(IssueTokenRequest request, String userId) {
        return fetchEvaluateAndMaybeRecordUsage(
                userId,
                request.instanceId(),
                info -> licenseValidationService.assertValid(info, request)
        );
    }
    ...
    private LicenseInfo fetchEvaluateAndMaybeRecordUsage(String userId,
                                                         String instanceId,
                                                         Consumer<LicenseInfo> validator) {
        LicenseInfo licenseInfo = userService
                .getUser(userId)
                .orElseThrow(() -> new LicenseNotFoundException(userId));
