...
public class UserAsyncServiceImpl implements UserAsyncService {
	...
	@Override
	@Async
	@Retryable(retryFor = { SocketTimeoutException.class, ConnectTimeoutException.class, 
			UnknownHostException.class, HttpHostConnectException.class }, 
			maxAttemptsExpression = "${retry.licenseServiceAsync.maxAttempts}", 
			backoff = @Backoff(delayExpression = "${retry.licenseServiceAsync.initialDelay}", 
			maxDelayExpression = "${retry.licenseServiceAsync.maxDelay}", 
			multiplierExpression = "${retry.licenseServiceAsync.multiplier}"))
