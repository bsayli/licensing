...
class LicenseServiceClientImplTest {
    ...
    void issueAccess_delegates() {
        ...
        ServiceResponse<LicenseAccessResponse> expected = mock(ServiceResponse.class);
        when(adapter.issueAccess(any(IssueAccessRequest.class))).thenReturn(expected);

        ServiceResponse<LicenseAccessResponse> actual = client.issueAccess(req);
    ...
    void validateAccess_delegates() {
        ...
        ServiceResponse<LicenseAccessResponse> actual = client.validateAccess(token, req);

        assertSame(expected, actual);
        ArgumentCaptor<String> tokenCap = ArgumentCaptor.forClass(String.class);
        ArgumentCaptor<ValidateAccessRequest> reqCap =
                ArgumentCaptor.forClass(ValidateAccessRequest.class);
