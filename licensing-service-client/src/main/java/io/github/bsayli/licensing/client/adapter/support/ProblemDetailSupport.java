...
public final class ProblemDetailSupport {
    ...
    public static ProblemDetail extract(ObjectMapper om, ClientHttpResponse response) {
        ...
        if (!isJson(snap.contentType())) {
            return ProblemDetailFallbacks.nonJson(snap.status(), snap.contentType(), snap.statusUnavailable());
        }
        ...
        try {
            ...
            log.warn(
                    ...
                    snap.body().length,
                    e);
            return ProblemDetailFallbacks.unparsable(snap.status(), snap.contentType(), snap.statusUnavailable(), e);
        }
    }
