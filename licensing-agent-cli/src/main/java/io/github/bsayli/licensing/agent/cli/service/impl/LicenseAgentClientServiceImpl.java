...
import io.github.bsayli.licensing.agent.cli.model.LicenseAccessRequest;
import io.github.bsayli.licensing.agent.cli.model.LicenseAgentClientProperties;
import io.github.bsayli.licensing.agent.cli.model.LicenseAgentErrorResponse;
import io.github.bsayli.licensing.agent.cli.model.LicenseToken;
...
public class LicenseAgentClientServiceImpl implements LicenseAgentClientService {

  private static final Logger log = LoggerFactory.getLogger(LicenseAgentClientServiceImpl.class);

  private final LicenseAgentClientProperties props;
  private final ObjectMapper mapper = new ObjectMapper().registerModule(new JavaTimeModule());

  public LicenseAgentClientServiceImpl(LicenseAgentClientProperties clientProperties) {
  ...
  public Integer validateLicense(
    ...
    try (CloseableHttpClient http =
      ...
      final ServiceResponse<LicenseToken> api = parseServiceResponse(response);

      if (api == null) {
        log.error("Response could not be parsed as ServiceResponse.");
        return 1;
      }
      ...
      log.error("Unexpected response without data.");
      return 1;

    } catch (HttpResponseException e) {
      log.error("HTTP error: status={}, reason={}", e.getStatusCode(), e.getReasonPhrase());
      logAgentErrorIfPresent(e);
      return 1;

    } catch (Exception e) {
      log.error("Unexpected error while calling Agent service", e);
      return 1;
    }
  }
  ...
  private ServiceResponse<LicenseToken> parseServiceResponse(String json) {
    try {
      return mapper.readValue(
              json,
              mapper
                      .getTypeFactory()
                      .constructParametricType(ServiceResponse.class, LicenseToken.class));
    } catch (Exception e) {
      log.error("Failed to parse ServiceResponse JSON", e);
      return null;
    }
  }

  private boolean isSuccess(ServiceResponse<LicenseToken> api) {
    return api != null && api.getData() != null && api.getData().licenseToken() != null;
  }

  private void logSuccess(ServiceResponse<LicenseToken> api) {
    log.info("License validated successfully.");
    log.info("Token: {}", api.getData().licenseToken());
  }

  private void logAgentErrorIfPresent(HttpResponseException e) {
    try {
      ...
      final LicenseAgentErrorResponse err =
              mapper.readValue(body, LicenseAgentErrorResponse.class);

      log.error("errorCode={}, message={}", err.errorCode(), err.message());
      ...
      log.error("Failed to parse AgentErrorResponse body", parseErr);
    }
  }
}