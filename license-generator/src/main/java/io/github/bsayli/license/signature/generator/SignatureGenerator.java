...
public final class SignatureGenerator {
  ...
  public static String createSignature(SignatureData payload)
    ...
    byte[] hash = sha256(json.getBytes(UTF8));
    PrivateKey pk = decodePrivateKeyFromBase64(PRIVATE_KEY_B64);
    byte[] sig = signHash(hash, pk);
  ...
  private static byte[] signHash(byte[] hash, PrivateKey privateKey)
    ...
    Signature signer = Signature.getInstance(SIG_SHA256_WITH_DSA);
  ...
  private static PrivateKey decodePrivateKeyFromBase64(String pkcs8Base64)
    ...
    KeyFactory kf = KeyFactory.getInstance(DSA_KEY_ALGORITHM);
